...
### Nhóm 8 — Tra lại 2026-09-13, từ một khung phân loại phương pháp SLM (nhánh B1: self-training bootstrap) do Bi đưa

Khung này xếp việc của Bi vào đúng **Nhánh B1** ("tự huấn luyện kiểu bootstrap trên các lời giải đã kiểm: lấy mẫu, lọc bằng bộ chấm đáp án, fine-tune, lặp lại"), cùng nhóm với STaR/ReST-EM đã biết. Tra 2 trích dẫn còn lại trong B1 chưa từng khảo sát:

| Bài | Tác giả | arXiv/venue | Ngày | Làm gì | Đụng câu hỏi của Bi? |
|---|---|---|---|---|---|
| Self-Training with Direct Preference Optimization Improves Chain-of-Thought Reasoning | Tianduo Wang, Shichen Li, Wei Lu | [2407.18248](https://arxiv.org/abs/2407.18248), ACL 2024 | 7/2024 | Verifier luật (regex + `eval()`, giống họ math-verify), 3 vòng self-training + DPO, model **nhỏ tới 250M** (Flan-T5-Base) tới 8B (Llama-3). | **Đối chiếu quan trọng nhất mới tìm được** — verifier luật khác, model còn nhỏ hơn cả 0,5B của Bi, nhưng **tăng đều không sụp** qua 3 vòng, kể cả ở 250M. Không đo pass@k xuyên vòng (chỉ vòng đầu), không đo hội tụ cách viết. Nếu số liệu A/B/C của Bi đứng vững, đây là chỗ nói rõ nhất trong Discussion: phần lớn self-training luật không sụp, của Bi thì không tăng — cần giải thích, không phải hiện tượng phổ biến. |
| Learning to Reason via Self-Iterative Process Feedback for Small Language Models | Kaiyuan Chen, Jin Wang, Xuejie Zhang | [2025.coling-main.203](https://aclanthology.org/2025.coling-main.203), COLING 2025 | 1/2025 | Verifier là **process reward model đã học** (không phải luật cố định), Gemma-2B, đo GSM8K/MBPP/MMLU_Math/HumanEval. | Không đụng thẳng — cơ chế chấm khác hẳn (PRM học được, không phải luật lệch-vì-định-dạng), không thấy đo pass@k theo vòng hay hội tụ chữ viết trong abstract. |

**Chốt 2026-09-13:** vẫn còn đất, không đổi kết luận "chưa có bài nào ghép đủ 3 mảnh". Việc cần làm: đưa `2407.18248` vào Related Work như đối chiếu chính — bài gần nhất cho thấy self-training luật ở model rất nhỏ thường KHÔNG sụp, làm nổi bật câu hỏi "vậy tại sao của mình không tăng" thay vì chỉ nói "verifier lệch không phải nguyên nhân".
